...
class LoRALinear(HalfLinear):
    ...
    def __init__(
        ...
        # Set the base half-precision weights to not train
        self.weight.requires_grad = False
        if self.bias is not None:
            self.bias.requires_grad = False

        # Define LoRA layers (float32)
        ...
        # Initialize lora_b to zero for identity initialization
        torch.nn.init.zeros_(self.lora_b.weight)
        

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_half = x.to(torch.float16)
        base_out = super().forward(x_half).to(torch.float32)

        # LoRA adapters work in float32
        lora_out = self.lora_b(self.lora_a(x))

        # Return sum of both, cast back to x.dtype (usually float32)
